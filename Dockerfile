...
# Remove big stuff we don't need
RUN rm -rf /usr/local/tomcat/webapps.dist ; rm -rf /lib/aarch64-linux-gnu/perl-base ; rm /bin/aarch64-linux-gnu-objdump ; rm /bin/aarch64-linux-gnu-as ; rm /bin/aarch64-linux-gnu-readelf ; rm /bin/aarch64-linux-gnu-ld.bfd ; rm /bin/aarch64-linux-gnu-dwp ; rm /bin/perl5.34.0 ; rm /bin/perl ; rm /bin/aarch64-linux-gnu-ld.gold ; rm -rf /lib/locale ; rm /lib/aarch64-linux-gnu/libopcodes-2.38-system.so ; rm /lib/aarch64-linux-gnu/libbfd-2.38-system.so ; rm /lib/aarch64-linux-gnu/libdb-5.3.so ; rm /lib/aarch64-linux-gnu/libctf.so.0.0.0 ; rm /lib/aarch64-linux-gnu/libctf-nobfd.so.0.0.0 ; rm /lib/aarch64-linux-gnu/libpng16.so.16.37.0 ; rm /lib/aarch64-linux-gnu/libblkid.so.1.1.0 ; rm /lib/aarch64-linux-gnu/libncursesw.so.6.3 ; rm /lib/aarch64-linux-gnu/libsemanage.so.2 ; rm /lib/aarch64-linux-gnu/libnettle.so.8.4 ; rm /lib/aarch64-linux-gnu/libhogweed.so.6.4 ; rm /lib/aarch64-linux-gnu/libfontconfig.so.1.12.0 ; rm /lib/aarch64-linux-gnu/libgssapi_krb5.so.2.2 ; rm /lib/aarch64-linux-gnu/libapt-private.so.0.0.0 ; rm /lib/aarch64-linux-gnu/libldap-2.5.so.0.1.11 ; rm /lib/aarch64-linux-gnu/libgmp.so.10.4.1 ; rm /lib/aarch64-linux-gnu/libbrotlienc.so.1.0.9 ; rm /lib/aarch64-linux-gnu/libsepol.so.2 ; rm /lib/aarch64-linux-gnu/libzstd.so.1.4.8 ; rm /lib/aarch64-linux-gnu/libfreetype.so.6.18.1 ; rm /lib/aarch64-linux-gnu/libkrb5.so.3.3 ; rm -rf /lib/aarch64-linux-gnu/gconv ; rm -rf /lib/aarch64-linux-gnu/ldscripts ; rm /bin/lscpu ; rm /bin/dpkg-split ; rm /bin/expr ; rm /bin/csplit ; rm /bin/sed ; rm /bin/aarch64-linux-gnu-gprof ; rm /bin/dpkg-divert ; rm /bin/ptx ; rm /bin/lsblk ; rm /bin/install ; rm /bin/vdir ; rm /bin/dir ; rm /bin/mawk ; rm /bin/aarch64-linux-gnu-objcopy ; rm /bin/aarch64-linux-gnu-strip ; rm /bin/trust ; rm /bin/curl ; rm /bin/gpgv ; rm /bin/localedef ; rm /bin/wget
